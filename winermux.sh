...
winedesktop() {
	...
	DISPLAY=:0 taskset -c 2-3 box64 $GLIBC_PREFIX/opt/wine/bin/wine explorer /desktop=shell,1280x800 cmd > /sdcard/winermux/wine_log.txt 2>&1
	pkill -f pulseaudio
	pkill -f app_process
	exit 0
}
	...
	export PATH=$GLIBC_PREFIX/bin:$PATH
	export WINEPREFIX=$GLIBC_PREFIX/opt/.wine
	box64 $GLIBC_PREFIX/opt/wine/bin/wine wineboot > /sdcard/winermux/wineboot_log.txt 2>&1
	winedesktop
	exit
	fi
	else
